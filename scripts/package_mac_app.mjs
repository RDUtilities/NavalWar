...
const archive = path.join(build, 'Naval War-development.zip');
...
try {
  ...
  run('ditto', ['-c', '-k', '--sequesterRsrc', '--keepParent', app, path.join(temporary, 'Naval War-development.zip')]);
  const extracted = path.join(temporary, 'extracted');
  run('ditto', ['-x', '-k', path.join(temporary, 'Naval War-development.zip'), extracted]);
  run('codesign', ['--verify', '--deep', '--strict', path.join(extracted, 'Naval War.app')]);
  const binary = 'Naval War.app/Contents/MacOS/NavalWar';
  ...
  await fs.copyFile(path.join(temporary, 'Naval War-development.zip'), archive);
  ...
  await fs.writeFile(path.join(build, 'package-verification.json'), JSON.stringify(report, null, 2) + '\n');
